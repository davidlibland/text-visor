...
test("Testing the probability of not rejecting symbols gained differential", () => {
    const differential = new ProbOfNotRejectingSymbolsGainedDifferential(0);
    expect(differential.evaluate("he", "heal")).toEqual(.75);
});

test("Testing the probability of not rejecting symbols gained differential", () => {
    const differential = new ProbOfNotRejectingSymbolsGainedDifferential(Math.log(0.1 / (0.9)));
