 ...
 */
export default abstract class AbstractQualityAssessor<
    S = string,
    T = string,
    E extends WeightedPrediction<T> = WeightedPrediction<T>> {
     ...
     */
    public abstract assess(input: S,
                           predictions: Array<(WeightedPrediction<T> & E)>,
                           limit: number,
                           offset: number,
                           qualityType: QualityModuleType,): Array<WeightedPrediction<T> & E>;
}